...
def funcionParaSalirAlMenuCLIENTE(ventana_):
	global casilleros, conexion_, REACTORBANDERACLIENTE
...
def mostrarTableroCLIENTE(ventana_, tablero):
	...
	opcionMenu = [ 
		Opcion('Menu', (650, 540), 'menu', funcionParaSalirAlMenuCLIENTE, fuenteParaMenu, ventana_)
					   ]

	opcionesVolverAJugar = [
		Opcion('Si', (200, 560), 'si', funcionParaVolverAJugarCLIENTE, fuenteParaVolver, ventana_),
		Opcion('No', (250, 560), 'no', funcionParaSalirAlMenuCLIENTE, fuenteParaVolver, ventana_)
									]
...
def clickTableroCLIENTE(tablero):
	...
	dibujarJugada(tablero, fila, col, casilla, casilleros[12]) #16/01/2019
...
def jugarContraJugadorCLIENTE(ventana_):
	#pygame.init()
	#ventana_ = pygame.display.set_mode((800,600))
	# ----------------------------------------------------------------
	global REACTORBANDERACLIENTE

	if REACTORBANDERACLIENTE:
...
def funcionDisparadoraCLIENT(ventana, texto):
	from twisted.internet import reactor, protocol, task
	global tick
	casilleros = [' '] * 20
	casilleros[12] = 'basura'
	...
	tick = task.LoopingCall(jugarContraJugadorCLIENTE, ventana)
	tick.start(1.0 / framesPerSecond)
...
def CLIENTATETI(ventana, texto):
	global REACTORBANDERACLIENTE, tc, tick
	import threading
	
	try:
		if tc:
			REACTORBANDERACLIENTE = True
			print 'Reutilizando thread cliente'
	except Exception as e:
		REACTORBANDERACLIENTE = True
		tc = threading.Thread(target=funcionDisparadoraCLIENT, args=(ventana, texto))
		tc.start()

	#REACTORBANDERACLIENTE = True
	#t = threading.Thread(target=funcionDisparadoraCLIENT, args=(ventana, texto))
	#t.start()

	while REACTORBANDERACLIENTE:
		pass
