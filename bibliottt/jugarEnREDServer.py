...
def funcionParaSalirAlMenuSERVENT(ventana_):
	global casilleros, REACTORBANDERA, conexion_
	try:
		casilleros[13] = False
		for i in range(1,20):
			casilleros[i] = ' '
...
def funcionParaVolverAJugarSERVENT(ventana_):
	...
	if casilleros[13]:
		funcionParaSalirAlMenuSERVENT(ventana_)
...
def clickTableroSERVENT(tablero):
	...
	dibujarJugada(tablero, fila, col, casilla, letraServer) #16/01/2019
...
class Conexiones(protocol.Protocol):
	...
	def connectionMade(self):
		...
		self.elTablero[15] = letraServer # agregado 16/01/2019
...
class Fabrica(protocol.Factory):
	...
	def __init__(self):
		global fabricaParaCerrar
		fabricaParaCerrar = self
...
def funcionDisparadoraSERVENT(ventana, letra_):
	from twisted.internet import reactor, protocol, task
	global letraServer, letraClient
	...
	#puerto = int(puerto_)
	#puertoCon = reactor.listenTCP(puerto, Fabrica())
	#puertocon.stopListening()
	framesPerSecond = 30.0
	tick = task.LoopingCall(jugarContraJugadorSERVENT, ventana)
...
def SERVENTATETI(ventana, letra_):
	...
	#import twisted
	#sys.path.append('/usr/lib/python2.7/dist-packages/twisted/internet/')
	#from twisted.internet import reactor
	...
	#procesoReactor = Process(target=reactor.run)
	#procesoReactor.start()
	#procesoReactor.join()
	#while REACTORBANDERA.value :
	#	pass
	#print 'paso111111111'
	#procesoReactor.terminate()
...
#	puerto = int(puerto_)
#	try:
#		reactor.listenTCP(puerto, Fabrica())
#	except:
#		pass
