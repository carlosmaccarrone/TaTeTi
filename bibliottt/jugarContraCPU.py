...
def traerTableroDuplicado(casillerosPC):
	tablaDuplicada = []

	for casilla in casillerosPC:
		tablaDuplicada.append(casilla)

	return tablaDuplicada

def movimientoAleatorioDeLista(casillerosPC, casillasProbar):
	posiblesMovimientos = []

	for casilla in casillasProbar:
		if esCasillaLibre(casillerosPC, casilla):
			posiblesMovimientos.append(casilla)

	if len(posiblesMovimientos) != 0:
		return random.choice(posiblesMovimientos)

def hacerUnMovimiento(duplicada, letra, casilla):
	duplicada[casilla] = letra

def traerMovimientoPC(casillerosPC, letraPC):
	if letraPC == 'X':
		letraJugador = 'O'
	else:
		letraJugador = 'X'
	...
	for casilla in range(1, 10):
		dup = traerTableroDuplicado(casillerosPC)
	...
	casilla = movimientoAleatorioDeLista(casillerosPC, [1, 3, 7, 9])
	if casilla != None:
		return casilla

	if esCasillaLibre(casillerosPC, 5):
		return 5

	return movimientoAleatorioDeLista(casillerosPC, [2, 4, 6, 8])
# --------------------------------------------------------------------------------
# --------------------------------------------------------------------------------
# --------------------------------------------------------------------------------
...
def jugarContraCPU(ventana_, letraJugador):
	...
	while juegoEnCurso:
		...
		for e in eventos:
			...
			if letraJugador == turno:
				...
				if not juegoGanado(tableroContraCPU) and not seLlenoTablero():
					casillaPC = traerMovimientoPC(casilleros, letraPC)
					...
					dibujarJugada(tableroContraCPU, fila, col, casillaPC, letraPC)
